...
test("a hung agent is killed and failed when it exceeds the wall-clock timeout", async () => {
  ...
  try {
    const config = defaultDispatchConfig();
    config.repoPath = paths.root;
    // Sleep far longer than the timeout; the registry must kill and fail it.
...
test("at the pane cap the next job queues, then reuses a freed pane on completion", async () => {
  ...
  try {
    ...
    // Finish j1's pane by hand: simulate its capture completing, then poke a tick
    // by finishing through the private path — we call the completion via the
    // capture file + a manual tick isn't exposed, so drive it through drainQueue
    // by finishing the pane the way the watcher would.
    // Write a sentinel into j1's capture and run one manual poll.
    await fsp.writeFile(j1.captureFile, "done\n__CO_DISPATCH_DONE__ 0\n", "utf8");
    await (reg as unknown as { tick: () => Promise<void> }).tick();

    // j1 is done; its pane freed and the queued j3 should now be running in it.
