...
export function toolDefinitions(opts: { dispatch?: boolean } = {}): Tool[] {
  ...
  if (opts.dispatch) {
    ...
    tools.push({
      name: "feature_land",
      description:
        "Land a finished feature into `dev` through a GitHub pull request: fetch, rebase its branch onto the current `origin/dev` tip, run the project's build+test on that combined state, push the branch, open (or refresh) its PR into `dev`, and open the Ctrl-O review gate showing the PR and the result. The gate IS the confirmation — the merge (`gh pr merge --merge`, a merge commit) happens ONLY if the captain presses [m] over a green result; a conflict or a red build+test is shown but not mergeable and is never pushed. On a merge the feature's worktree is torn down and its branch ref kept. `dev` is never written locally. Needs an interactive terminal (the gate); reports cleanly if there is none, if a crew agent is still working the feature, or if a prerequisite is missing (no gh, not authenticated, no `origin/dev`).",
      input_schema: {
        type: "object",
        properties: {
    ...
    tools.push({
      ...
      description:
        "DO NOT reach for this to merge a ready head — merging is not yours to do. When the queue head is `ready`, the captain's Ctrl-O queue tab is already showing its pull request and build+test result with a live [m], and pressing it merges that PR on GitHub, tears down its worktree, advances the queue and processes the next head, none of which involves you. In an interactive session this tool merges NOTHING: it returns the head's current state immediately and tells you the [m] is live. It exists only as a fallback for a session with no panel at all (piped/non-TTY), where there is no key to press and a ready head is merged directly. Use feature_list/feature_status to report queue state instead. Never say a feature landed because you called this, and never wait on the captain's keystroke — you are not in that loop.",
      input_schema: { type: "object", properties: {}, additionalProperties: false },
    });
    tools.push({
      name: "feature_resolve_head",
      description:
        "Dispatch a FRESH crew agent to unblock the current merge-queue HEAD when it is `blocked` by a rebase conflict or a red build+test. The agent runs in the FEATURE'S OWN isolated worktree, on its `co/feat-<slug>` branch (never `dev`): it fetches, rebases onto the current `origin/dev` tip, resolves the conflict, makes the combined build+test pass, and commits — it does NOT push, open a PR, or merge. Like every dispatch this is CONFIRM-GATED: it ARMS the resolver order (shown to the captain with the target worktree) and fires only when the captain types `confirm`; it launches nothing on its own. While the agent works, the head is `resolving`; when it finishes the head is automatically re-processed (rebase + build+test) and becomes `ready` if green or `blocked` again if not. Bounded to a few attempts per head; after the limit the head stays blocked for the captain to resolve by hand or abandon. Use this on a blocked head instead of resolving conflicts yourself.",
      input_schema: { type: "object", properties: {}, additionalProperties: false },
    });
    tools.push({
      name: "feature_list",
