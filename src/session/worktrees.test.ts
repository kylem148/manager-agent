 ...
 * tests that need a real install to run.
 *
...
async function makeRepo(extra: { nodePackage?: boolean } = {}): Promise<{
  root: string;
  repo: string;
  base: string;
  forge: FakeForge;
  ...
  if (extra.nodePackage) {
    ...
    const lock = spawnSync("npm", ["install", "--package-lock-only", "--silent"], {
      cwd: repo,
      encoding: "utf8",
    });
