...
export interface LandingQueueEntry {
  feature: string;
  /** The most recent prepareLanding outcome; absent until one has run. A
   *  recorded green goes stale the moment another feature lands (dev moves) —
   *  executeLanding refuses it then, and the cure is another prepare. */
  prepared?: PrepareResult;
}

/**
 * The ordered record of features awaiting landing. In-memory and per-session,
 * like the registry's job map. Landing is serial by nature — each prepare
 * rebases onto whatever dev is at that moment — so head() is the one sensible
 * candidate to prepare next; remove() a feature once it lands (or is
 * abandoned). The gate slice owns driving this; the queue just remembers.
 */
export class LandingQueue {
  private readonly entries: LandingQueueEntry[] = [];

  /** Add a feature to the back of the queue. Idempotent: an already-queued
   *  feature keeps its position and its recorded prepare result. */
  enqueue(feature: string): LandingQueueEntry {
    const existing = this.get(feature);
    if (existing) return existing;
    const entry: LandingQueueEntry = { feature };
    this.entries.push(entry);
    return entry;
  }

  /** Snapshot in landing order (the entries themselves, not copies). */
  list(): LandingQueueEntry[] {
    return [...this.entries];
  }

  /** The next feature to land, by queue order. */
  head(): LandingQueueEntry | undefined {
    return this.entries[0];
  }

  get(feature: string): LandingQueueEntry | undefined {
    return this.entries.find((e) => e.feature === feature);
  }

  /** Attach a prepare outcome to a queued feature. Throws for a feature that
   *  isn't queued — preparing something outside the queue is a caller bug. */
  recordPrepare(feature: string, result: PrepareResult): LandingQueueEntry {
    const entry = this.get(feature);
    if (!entry) throw new Error(`feature '${feature}' is not queued for landing`);
    entry.prepared = result;
    return entry;
  }

  /** Drop a feature (landed or abandoned). Returns whether it was queued. */
  remove(feature: string): boolean {
